...
static source_t *
find_source (const char *file_name)  /** for example, file_name="test.c" */
{
    source_t *src;
    ...
    for (src = sources; src; src = src->next)
        if (!strcmp (file_name, src->name))  /** if the same filename, then return */
            return src;

    src = xcalloc (1, sizeof (source_t));
    src->name = xstrdup (file_name);  /** this name is also "test.c" */
    src->coverage.name = src->name;  /** this name is also "test.c" */
    src->index = sources ? sources->index + 1 : 1;
    src->next = sources;  /** insert src into the head of sources list */
    sources = src;

    return src;  /** in fact, src is the head node of sources list */
}
...
read_graph_file (void)
{
    ...
    if (!gcov_magic (gcov_read_unsigned (), GCOV_NOTE_MAGIC))  /** read magic, "gcno", 4Bytes */
    {
    ...
    version = gcov_read_unsigned ();  /** read version, 4Bytes */
    ...
    bbg_stamp = gcov_read_unsigned ();  /** read stamp, 4Bytes */

    while ((tag = gcov_read_unsigned ()))  /** read tag, 4Bytes */
    {
        unsigned length = gcov_read_unsigned ();  /** read its length, 4Bytes */
        gcov_position_t base = gcov_position ();

        if (tag == GCOV_TAG_FUNCTION)  /** it is a FUNCTION tag */
        {
            char *function_name;
            ...
            ident = gcov_read_unsigned ();  /** read function identifier, 4Bytes */
            checksum = gcov_read_unsigned ();  /** read the checksum, 4Bytes */
            function_name = xstrdup (gcov_read_string ());  /** read the string, in general, function name is main() */
            src = find_source (gcov_read_string ());  /** read the string, in general, it is main() */
            lineno = gcov_read_unsigned ();  /** read line number, 4Btyes */

            /** allocate a function_t structure to save the information of the current FUNCTION tag */
            ...
            /** insert fn into the head of functions list */
            fn->next = functions;
            functions = fn;
            current_tag = tag;
        ...
        else if (fn && tag == GCOV_TAG_BLOCKS)  /** it is a BLOCKS tag */
        {
            if (fn->blocks)
            ...
            else
            {
                ...
                /** allocate a space save by fn->blocks pointer, and read these blocks info */
                fn->blocks = xcalloc (fn->num_blocks, sizeof (block_t));
                for (ix = 0; ix != num_blocks; ix++)  /** read num_blocks of words, that is, num_blocks * 4 Bytes */
                    fn->blocks[ix].flags = gcov_read_unsigned ();
            }
        }
        else if (fn && tag == GCOV_TAG_ARCS)  /** it is a BLOCKS tag */
        {
            unsigned src = gcov_read_unsigned ();  /** read the source block */
            ...
            {
                struct arc_info *arc;
                unsigned dest = gcov_read_unsigned ();  /** read the destination block */
                unsigned flags = gcov_read_unsigned ();  /** read the block flags */

                if (dest >= fn->num_blocks)
                    goto corrupt;
                arc = xcalloc (1, sizeof (arc_t));  /** allocate a space to save the arc between two Basic Blocks */

                arc->dst = &fn->blocks[dest];  /** set destination Block */
                arc->src = &fn->blocks[src];  /** set source Block */
                ...
                /** insert arc into the head of fn->blocks[src], which is an array with succ/pred pointer */
        ...
        else if (fn && tag == GCOV_TAG_LINES)  /** it is a LINES tag */
        {
            unsigned blockno = gcov_read_unsigned ();  /** read the block NO., 4 Bytes */
            ...
            for (ix = 0; ;  )
            {
                unsigned lineno = gcov_read_unsigned ();  /** read the line NO., 4 Bytes */
                ...
                else  /** lineno=0, then, a filename must following it */
                {
                    const char *file_name = gcov_read_string ();  /** read the string - filename */

                    if (!file_name)  /** if filename is NULL, then, break */
                        break;
                    src = find_source (file_name);  /** else, find the source in sources list, or allocate a source */
...
read_count_file (void)
{
    ...
    if (!gcov_magic (gcov_read_unsigned (), GCOV_DATA_MAGIC))  /** read magic, "gcda", 4Bytes */
    {
...
cleanup:
    ...
    tag = gcov_read_unsigned ();  /** read stamp, 4Bytes */
    if (tag != bbg_stamp)
    {
    ...
    while ((tag = gcov_read_unsigned ()))  /** read tag, 4Bytes */
    {
        unsigned length = gcov_read_unsigned ();  /** read length, 4Bytes */
        unsigned long base = gcov_position ();

        if (tag == GCOV_TAG_OBJECT_SUMMARY)
            gcov_read_summary (&object_summary);
        else if (tag == GCOV_TAG_PROGRAM_SUMMARY)  /** in gcov program, it will omit PROGRAM summary */
            program_count++;
        else if (tag == GCOV_TAG_FUNCTION)
        {
            unsigned ident = gcov_read_unsigned ();  /** read FUNCTION ident, 4Bytes */
            ...
            if (!fn)
                ;
            else if (gcov_read_unsigned () != fn->checksum)  /** read FUNCTION checksum, 4Bytes */
            {
mismatch:
                ;
        ...
        else if (tag == GCOV_TAG_FOR_COUNTER (GCOV_COUNTER_ARCS) && fn)
        {
            ...
            for (ix = 0; ix != fn->num_counts; ix++)
                fn->counts[ix] += gcov_read_counter ();  /** read counter, 8 Bytes for each */
        }
        gcov_sync (base, length);
    ...
    for (ix = 0, blk = fn->blocks; ix != fn->num_blocks; ix++, blk++)
    {
        ...
        if (out_of_order)
        {
            ...
            while (changes)
            {
                ...
                changes = 0;
                /** insertion sort for the Chain of exit and entry arcs of the current block */
