...
/* zubo modified for compiling, add header file stddef.h */
#include <stddef.h>
#include "gcov-io.h"
...
gcov_allocate (unsigned length)
{
    ...
    if (!new_size)
        new_size = GCOV_BLOCK_SIZE;  /***** if new_size==0, then, new_size=1024 */
...
gcov_read_words (unsigned words)
{
    ...
    unsigned excess = gcov_var.length - gcov_var.offset;  /** excess is the number of words which can be excessed */
    ...
    if (excess < words)
    {
        ...
        if (gcov_var.length + words > gcov_var.alloc)
            gcov_allocate (gcov_var.length + words);  /** allocate space, the space pointer is saved in gcov_var.buffer */
...
{
    unsigned length = gcov_read_unsigned ();  /** read the length of string, 4Bytes */

    if (!length)
        return 0;

    return (const char *) gcov_read_words (length);  /** read 'length' number of words , length * 4Bytes */
}
#endif
...
gcov_read_summary (struct gcov_summary *summary)
{
    ...
    for (csum = summary->ctrs, ix = GCOV_COUNTERS_SUMMABLE; ix--; csum++)
    {
        ...
        csum->sum_all = gcov_read_counter ();  /***** 8Bytes *****/
        csum->run_max = gcov_read_counter ();  /***** 8Bytes *****/
        csum->sum_max = gcov_read_counter ();  /***** 8Bytes *****/
    }
