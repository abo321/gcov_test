...
print_prefix (const char *filename, unsigned depth, gcov_position_t position)
{
    ...
    /***** print the previous characters of prefix, that is, some spaces, the number is depth */
...
found:
        ...
        if (format->proc)
            (*format->proc) (filename, tag, length);  /***** call the procedure of this tag stored in tag_table */
              ...
              unsigned tag ATTRIBUTE_UNUSED, unsigned length ATTRIBUTE_UNUSED)
{
    ...
    if (gcov_position () - pos < length)
    {
        ...
        name = gcov_read_string ();  /** read function name */
        printf (", `%s'", name ? name : "NULL");
        name = gcov_read_string ();  /** read source file name */
        printf (" %s", name ? name : "NULL");
        printf (":%u", gcov_read_unsigned ());  /** read lineno of the function in the source file */
    }
    filename = filename;
    tag = tag;
}
             ...
             unsigned tag ATTRIBUTE_UNUSED, unsigned length ATTRIBUTE_UNUSED)
{
    ...
    unsigned count = gcov_read_summary (&summary);
